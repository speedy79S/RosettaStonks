...
export interface Service {
    ...
    decreaseTime(time: Date): Promise<void>; // New method for decreasing time
...
export class FluencyBuilderService implements Service {
    ...
    async addTime(time: Date): Promise<void> {
        ...
        if (req === undefined || req.body === null) throw Error("Could not add time");
    ...
    async decreaseTime(time: Date): Promise<void> {
        const req = await getRequest(FluencyBuilderTimeRequestKey);
        if (req === undefined || req.body === null) throw Error("Could not decrease time");

        const body = JSON.parse(req.body);
        for (let i = 0; i < body.variables.messages.length; i++) {
            const msg = body.variables.messages[i];
            msg.durationMs = Math.max(msg.durationMs - time.getTime(), 0); // Prevent negative time
        }
        req.body = JSON.stringify(body);
...
export class FoundationsService implements Service {
    ...
    async decreaseTime(time: Date): Promise<void> {
        const req = await getRequest(FoundationsTimeRequestKey);
        if (req === undefined) throw Error("Could not decrease time");

        const currentTimeRequests = this.getTimeRequests(req, time);
        const remainingTimeMs = Math.max(currentTimeRequests[0].body.getTime() - time.getTime(), 0); // Prevent negative time
        const requests = this.getTimeRequests(req, new Date(remainingTimeMs));

        console.debug("sending decreased time requests", requests);
        const promises = requests.map((req) =>
            fetch(req.url, {
                method: req.method,
                headers: req.headers,
    ...
    async validateLesson(): Promise<void> {
        ...
        if (req === undefined) throw Error("Could not validate lesson");
